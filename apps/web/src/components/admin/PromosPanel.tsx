...
const TYPE_LABELS: Record<PromoCodeType, string> = {
  VIP_DAYS: 'أيام VIP',
  COINS: 'نقاط',
  CROSS_BORDER_ACCESS: 'الزواج الدولي',
};
...
export function PromosPanel() {
  ...
  async function refresh() {
    ...
    try {
      ...
      setError('تعذّر جلب الأكواد الترويجية');
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div className="flex flex-col gap-6">
      <form onSubmit={onSubmit} className="rounded-2xl border border-blue-100 bg-surface p-4 shadow-sm">
        <h3 className="font-semibold text-blue-900">إنشاء كود ترويجي جديد</h3>
          ...
          <table className="w-full min-w-[640px] text-sm">
            <thead>
              <tr className="border-b border-blue-100 text-right text-ink-500">
                ...
                <th className="px-4 py-3 font-medium">توثيق</th>
            ...
            <tbody>
              {items.map((item) => (
                <tr key={item._id} className="border-b border-blue-50 last:border-0">
                  ...
                  <td className="px-4 py-3">{item.requiresVerification ? 'نعم' : 'لا'}</td>
                  <td className="px-4 py-3">
                    <span
