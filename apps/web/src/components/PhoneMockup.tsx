...
import { brandNameFor } from '@/lib/seo';
...
export function PhoneMockup({ className = '' }: { className?: string }) {
  ...
  const dict = LANDING_DICTIONARY[locale].phoneMockup;

  const MINI_FEATURES = [
    { icon: ShieldCheckIcon, label: dict.miniFeatures[0] },
    { icon: LockClosedIcon, label: dict.miniFeatures[1] },
    { icon: HeartIcon, label: dict.miniFeatures[2] },
  ];
        ...
        <div className="relative overflow-hidden rounded-[2.1rem] bg-white">
          ...
          <div className="flex flex-col gap-4 px-5 pb-6 pt-8">
            <div className="flex flex-col items-center gap-1.5 text-center">
              ...
              <span className="font-display text-sm font-bold text-blue-900">{brandNameFor(locale)}</span>
            </div>

            <div className="mt-1 rounded-2xl bg-blue-50 p-3.5">
              <p className="text-center text-[11px] font-semibold text-blue-900">{dict.chooseLabel}</p>
              <div className="mt-2.5 grid grid-cols-2 gap-2">
                <div className="rounded-xl border border-blue-200 bg-white py-2.5 text-center">
                  <span className="text-[10px] font-semibold text-blue-700">{dict.searchHusband}</span>
                </div>
                <div className="rounded-xl border-2 border-rose-400 bg-rose-50 py-2.5 text-center">
                  <span className="text-[10px] font-semibold text-rose-600">{dict.searchWife}</span>
                </div>
              </div>
            </div>
            ...
            <div className="mt-1 flex flex-col gap-1.5">
              <p className="text-[10px] font-semibold text-blue-900/70">{dict.whyTitle}</p>
