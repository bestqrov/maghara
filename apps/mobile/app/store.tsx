...
export default function StoreScreen() {
  ...
  const { locale, dict, row, textAlign } = useAppDict();
  ...
  > = {
    PENDING: {
      label: dict.store.statusPending,
      bg: colors.gold100,
      fg: colors.emerald900,
    },
    SUCCESS: {
      label: dict.store.statusSuccess,
      bg: colors.emerald50,
      fg: colors.emerald700,
    },
    FAILED: {
      label: dict.store.statusFailed,
      bg: colors.rose100,
      fg: '#dc2626',
    },
  };
  ...
  function openStoreOnWebsite() {
    Linking.openURL(websiteUrl(locale, '/store'));
  }
  ...
  return (
    <FlatList
      ...
      data={transactions}
      keyExtractor={(t: Transaction) => t._id}
      ListHeaderComponent={
        <View style={{ gap: 14, marginBottom: 8 }}>
          <NavBar />
          <Text style={[styles.title, { textAlign }]}>{dict.store.title}</Text>
          <Text style={[styles.notice, { textAlign }]}>
            {dict.store.websiteNotice}
          </Text>

          <View style={styles.card}>
            <Text style={[styles.cardTitle, { textAlign }]}>
              {dict.store.coinPackagesTitle}
            </Text>
              ...
              {COIN_PACKAGES.map((pkg, i) => (
                <View key={pkg.coins} style={styles.coinPkg}>
                  ...
                  <Text style={styles.coinPrice}>
                    {dict.store.coinPriceLabels[i] ?? pkg.priceLabel}
                  </Text>
                </View>
              ))}
            </View>
          ...
          <View style={styles.vipCard}>
            <Text style={[styles.vipTitle, { textAlign }]}>
              {dict.store.vipPlanLabel} 👑
            </Text>
            <Text style={[styles.vipSubtitle, { textAlign }]}>
              {dict.store.vipPerks}
            </Text>
            <Text style={[styles.vipPrice, { textAlign }]}>
              {dict.store.vipPricePerMonth}
            </Text>
            <Button variant="gold" onPress={openStoreOnWebsite}>
              {dict.store.upgradeNow}
            </Button>
          </View>

          <Text style={[styles.cardTitle, { textAlign }]}>
            {dict.store.historyTitle}
          </Text>
          {transactions.length === 0 && (
            <Text style={styles.empty}>{dict.store.historyEmpty}</Text>
          )}
        </View>
      }
      renderItem={({ item }: { item: Transaction }) => (
        <View style={[styles.txRow, { flexDirection: row }]}>
          <View>
            <Text style={[styles.txType, { textAlign }]}>
...
const styles = StyleSheet.create({
  ...
  notice: { fontSize: 12, color: colors.ink500 },
